...
export class DetailComponent implements OnInit {
  ...
  getProduct() {
    this.productService.getProduct(this.id).subscribe(next => {
      this.product = next;
    }, error => {
      Swal.fire({
        text: 'Sản phẩm đã bị xóa, hoặc không tồn tại',
        icon: 'error',
  ...
  getAllCart() {
    if (this.tokenStorageService.getToken()) {
      const subscription = this.productService.getAllCart(this.username).subscribe(cartes => {
        this.carts = cartes;
      }, error => {
